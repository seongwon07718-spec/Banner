    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if lic_type == "7D":
                ...
                expires = now + dt.timedelta(days=30); lic_label = "30일"
            elif lic_type == "PERM":
                expires = None; lic_label = "영구"
            else:
...
class SimpleBannerView(discord.ui.View):
    ...
    async def setting_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not ok:
            ...
            return await interaction.response.send_message(
                embed=make_embed("설정 불가", "유효한 라이선스가 있어야 설정할 수 있어요. 라이선스를 등록하거나 갱신해줘.", COLOR_BLACK, fields),
                ephemeral=True
            )
    ...
    async def info_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            if lic_type == "영구":
                fields = [("종류", "영구", True), ("등록일", act_fmt, True)]
                embed = make_embed("라이선스 정보", "", COLOR_BLACK, fields)
            else:
                exp = parse_dt(expires_at)
                if not exp or (exp - now_utc()).total_seconds() <= 0:
                    fields = [("등록일", act_fmt, True), ("만료일", exp.strftime("%Y-%m-%d %H:%M") if exp else "-", True)]
