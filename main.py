import os, re, asyncio, subprocess, signal
import discord
...
def dec_stock_exact(s: Session, need: int) -> tuple[bool, str]:
    ...
    if cur < need: return False, f"재고가 부족해. 현재 재고는 {cur} R$야."
    st.total_stock_rbx = cur - need; s.commit(); return True, "OK"
...
async def 버튼패널(inter: discord.Interaction):
    try: await inter.response.defer(thinking=False, ephemeral=False)
    except: pass
...
async def 수동충전(inter: discord.Interaction, 유저: discord.User, 금액: int):
    ...
    s = db()
    try: u = ensure_user(s, str(유저.id)); u.balance += 금액; s.commit()
    finally: s.close()
    try: await 유저.send(f"{금액:,}원 충전 완료! 현재 잔액 반영됐어.")
    except: pass
...
async def 계좌수정(inter: discord.Interaction, 은행: str, 계좌: str, 예금주: str):
    ...
    s = db()
    try: st = get_settings(s); st.bank_name=은행; st.account_number=계좌; st.holder=예금주; s.commit()
    finally: s.close()
...
@app_commands.choices(모드=[app_commands.Choice(name="설정(덮어쓰기)", value="set"),
...
async def on_interaction(inter: discord.Interaction):
    ...
    try:
        if not inter.response.is_done(): await inter.response.defer(thinking=False, ephemeral=False)
    except: pass
    cid = inter.data.get("custom_id")

    if cid == "myinfo":
        s = db()
        try: u = ensure_user(s, str(inter.user.id))
        finally: s.close()
        try: await inter.user.send(f"누적 금액: {u.total_spent:,}원\n잔액: {u.balance:,}원\n등급: {u.tier}")
        except: pass
        return
    ...
    if cid == "topup":
        class BankModal(discord.ui.Modal, title="계좌이체 신청"):
            ...
            async def on_submit(self, i2: discord.Interaction):
                ...
                except: return await i2.response.send_message("금액 형식이 올바르지 않아.", ephemeral=True)
                s2 = db()
                try:
    ...
    if cid.startswith("topup_approve_") or cid.startswith("topup_reject_"):
        is_ok = cid.startswith("topup_approve_"); tid = int(cid.split("_")[-1])
        if not is_admin(inter): return
        s = db()
        try:
            ...
            if is_ok:
                u.balance += t.amount; t.status = "approved"; s.commit()
                try: user = await bot.fetch_user(int(t.discord_id)); await user.send(f"충전 완료! {t.amount:,}원 반영.\n현재 잔액: {u.balance:,}원")
                except: pass
            else:
                t.status = "rejected"; s.commit()
                try: user = await bot.fetch_user(int(t.discord_id)); await user.send("충전 요청이 거부되었어. 문의는 티켓으로 부탁해!")
                except: pass
        finally: s.close()
        return
    ...
    if cid == "buy":
        class BuyModal(discord.ui.Modal, title="로벅스 구매 신청"):
            ...
            async def on_submit(self, i2: discord.Interaction):
                m = str(self.method.value).strip(); n = str(self.nick.value).strip()
                s2 = db()
                try: o = Order(discord_id=str(i2.user.id), method=m, roblox_nick=n, status="requested"); s2.add(o); s2.commit()
                finally: s2.close()
                await i2.response.send_message("신청 접수! DM을 확인해줘.", ephemeral=True)
                try: user = await bot.fetch_user(int(i2.user.id)); await user.send("구매할 로벅스 수량을 보내주세요. 숫자만 보내주세요.")
                except: pass
...
async def on_message(message: discord.Message):
    ...
    if isinstance(message.channel, discord.DMChannel):
        ...
        if not re.fullmatch(r"\d+", txt or ""): return await message.channel.send("숫자만 입력해줘!")
        qty = int(txt); s = db()
        try:
            o = s.query(Order).filter_by(discord_id=str(message.author.id), status="requested")\
                              .order_by(Order.created_at.desc()).first()
            if not o:
                s.close(); return await message.channel.send("진행 중인 구매 신청이 없어. 다시 버튼으로 신청해줘.")
            o.amount_rbx = qty
            ok, msg = dec_stock_exact(s, qty)
            if not ok:
                o.status = "requested"; s.commit(); s.close(); return await message.channel.send(msg)
            o.status = "queued"; s.commit()
            st = get_settings(s); secure_ch_id = int(st.secure_channel_id or 0) or SECURE_CHANNEL_ID
            method, nick = o.method, o.roblox_nick
        finally: s.close()
        await message.channel.send("확인했어! 조금만 대기해주세요.")
        if secure_ch_id:
            ch = bot.get_channel(secure_ch_id) or await bot.fetch_channel(secure_ch_id)
            embed = discord.Embed(title="구매 확인 요청",
...
async def start_discord():
    if not DISCORD_TOKEN:
        print("DISCORD_TOKEN이 비어있어. 환경변수에 설정해줘."); return
...
async def main_async():
    ...
    try:
        ...
        try: web.send_signal(signal.SIGINT)
        except: pass
